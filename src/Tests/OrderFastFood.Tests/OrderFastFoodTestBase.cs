...
using OrderFastFood.Users;
using Castle.MicroKernel.Registration;
using Effort;
using EntityFramework.DynamicFilters;

namespace OrderFastFood.Tests
{
    public abstract class OrderFastFoodTestBase : AbpIntegratedTestBase<OrderFastFoodTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected OrderFastFoodTestBase()
        {
        ...
        protected void UsingDbContext(Action<OrderFastFoodDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<OrderFastFoodDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<OrderFastFoodDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<OrderFastFoodDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<OrderFastFoodDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OrderFastFoodDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<OrderFastFoodDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OrderFastFoodDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<OrderFastFoodDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<OrderFastFoodDbContext, Task<T>> func)
        {
            T result;
